...
function HomeAssistantClient (session) {
  ...
  this.extractBrightness = (device) => {
    ...
    const brightnessKeys = ['bright_value', 'bright_value_1', 'brightness', 'dimmer']
  ...
  this.extractColor = (device) => {
    ...
    const colorKeys = ['colour_data', 'color_data', 'colour', 'color', 'hsv', 'rgb']
  ...
  this.getScenes = async () => {
    try {
      ...
      if (payload && payload.devices) {
        // Filter only scene devices
        const scenes = payload.devices
          .filter(device => device.dev_type === 'scene')
          .map(scene => ({
            id: scene.id,
            name: JSON.parse(`"${scene.name}"`),
            type: scene.dev_type,
            icon: scene.icon,
            data: scene.data
          }))
  ...
  this.setColor = async (deviceId, hue, saturation, value) => {
    ...
    const h = Math.round((hue / 360) * 65535).toString(16).padStart(4, '0')
    const s = Math.round((saturation / 100) * 65535).toString(16).padStart(4, '0')
    const v = Math.round((value / 100) * 65535).toString(16).padStart(4, '0')
  ...
  this.setColorTemperature = async (deviceId, temperature) => {
    return this.deviceControl(deviceId, 'colorTemperatureSet', temperature, 'value')
  }
