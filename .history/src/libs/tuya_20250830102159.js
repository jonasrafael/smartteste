...
function HomeAssistantClient(session) {
  ...
  this.deviceDiscovery = async (retryCount = 0) => {
    ...
    console.log(
      `[DEVICE DISCOVERY] Starting discovery (attempt ${retryCount + 1}/${
        maxRetries + 1
      })`
    );
    console.log(
      `[DEVICE DISCOVERY] Session token:`,
      session?.token?.access_token ? "Present" : "Missing"
    );
    ...
    try {
      ...
      console.log(
        `[DEVICE DISCOVERY] Response headers:`,
        discoveryResponse.headers
      );
      ...
      if (
        ...
        console.log(
          `[DEVICE DISCOVERY] Service unavailable, retrying in ${
            retryDelay / 1000
