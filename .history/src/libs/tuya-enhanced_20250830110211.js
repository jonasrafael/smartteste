...
function HomeAssistantClient(session) {
  ...
  this.startRealTimeMonitoring = async (callback) => {
    try {
      ...
      this.logSystemEvent('error', 'Failed to start real-time monitoring', { error: err.message });
      return false;
    }
  };
  ...
  this.updateDeviceStates = async (callback) => {
    try {
      ...
      this.logSystemEvent('error', 'Failed to update device states', { error: err.message });
    }
  };
  ...
  this.getDeviceStates = async () => {
    try {
      ...
      this.logSystemEvent('error', 'Failed to get device states', { error: err.message });
      return [];
    }
  };
  ...
  this.parseDeviceProperties = (status) => {
    ...
    try {
      ...
      Object.keys(status).forEach(key => {
        if (!['state', 'online', 'brightness', 'color', 'colorTemperature', 'workMode', 'power'].includes(key)) {
  ...
  this.parseColorValue = (colorData) => {
    try {
      if (typeof colorData === 'string') {
        ...
        if (parsed.hue !== undefined && parsed.saturation !== undefined && parsed.value !== undefined) {
          return parsed;
        }
      }
  ...
  this.detectStateChanges = (deviceId, currentState) => {
    try {
      const previousState = JSON.parse(localStorage.getItem(`tuya_device_${deviceId}_previous_state`)) || {};
      const changes = [];
      ...
      localStorage.setItem(`tuya_device_${deviceId}_previous_state`, JSON.stringify(currentState));
    ...
    try {
      ...
      if (deviceState.changes && deviceState.changes.length > 0) {
        for (const change of deviceState.changes) {
          ...
          this.logSystemEvent('info', `Device ${deviceState.deviceName} ${change.type} changed`, {
  ...
  this.updateDeviceRecord = async (deviceState) => {
    try {
      ...
      const allRecords = JSON.parse(localStorage.getItem('tuya_device_records')) || [];
      allRecords.push(record);
  ...
  this.getDeviceUsageStats = async (deviceId, period = 'day') => {
    try {
      ...
      this.logSystemEvent('info', `Retrieved usage stats for device ${deviceId}`, {
        deviceId,
        period,
        stats
      });
      ...
      this.logSystemEvent('error', `Failed to get usage stats for device ${deviceId}`, { error: err.message });
      return null;
    }
  };
