...
function HomeAssistantClient(session) {
  ...
  this.deviceControl = async (deviceId, action, fieldValue, fieldName) => {
    ...
    try {
      ...
      let errorMessage = "Erro desconhecido ao controlar dispositivo";
      
      if (error.message) {
        // Check for specific error patterns
        if (error.message.includes("offline") || error.message.includes("unavailable")) {
          errorMessage = "Dispositivo está offline ou fora da rede WiFi";
        } else if (error.message.includes("timeout") || error.message.includes("time out")) {
          errorMessage = "Dispositivo não respondeu (timeout) - verifique se está conectado";
        } else if (error.message.includes("unauthorized") || error.message.includes("permission")) {
          errorMessage = "Sem permissão para controlar este dispositivo";
        } else if (error.message.includes("not found") || error.message.includes("invalid")) {
          errorMessage = "Dispositivo não encontrado ou ID inválido";
        } else if (error.message.includes("rate limit") || error.message.includes("too many")) {
          errorMessage = "Muitas tentativas - aguarde um momento antes de tentar novamente";
        } else if (error.message.includes("service unavailable") || error.message.includes("temporarily")) {
      ...
      enhancedError.fieldValue = fieldValue;
      enhancedError.fieldName = fieldName;
